...
def startBot(API_TOKEN):
    ...
    def handler_message(message):
        if message.content_type == "text":
            ...
            if prohibited == True:
                log_and_print("info", f"Обнаружен запрещенный контент от {message.from_user.username} (@{message.from_user.username}):\n" 
                              + message_text)
                try:
                    bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)
                    log_and_print("info", f"Сообщение {message.from_user.username} (@{message.from_user.username}) удалено.")
                except Exception as e:
                    log_and_print("error", f"Не удалось удалить сообщение {message.from_user.username}: {e}")
